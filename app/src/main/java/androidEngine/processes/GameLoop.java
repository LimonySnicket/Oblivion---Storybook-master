...
public class GameLoop extends Thread {
    ...
    //Constructors
    public GameLoop(ThreadManager t, int maxFrames, boolean mode) {
        threadManager = t;
        setMaxFPS(maxFrames);
        powerSavingMode = mode;
    }

    public GameLoop(ThreadManager t) {
        threadManager = t;
        setMaxFPS(100);
    ...
    private void checkInput() {
        threadManager.checkInput();
    }
    private void tick(double updateIncrament) {
        totalFrames++;
        threadManager.updateGame(updateIncrament / 100);
    }
